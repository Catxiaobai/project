    ...
    <el-card>
      ...
      <div id="table">
        <el-table :data="tableData" border style="width: 100%" @selection-change="handleSelection">
          ...
          <el-table-column prop="element" label="类别" width="180" :filters="filterData" :filter-method="filterType"> </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleImport(code, file) {
      console.log('test')
      this.$http
        .post(this.Global_Api + '/api/import_scenes', { name: file.name, type: 'sub', item: this.itemInfo })
        .then(response => {
          if (response.data.error_code === 0) {
