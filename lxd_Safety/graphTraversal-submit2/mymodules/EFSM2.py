    ...
    def findPathWithRepeat(self):
        ...
        while (len(tempPathList)>step) and (pathLength <= maxPathLength):
            ...
            if (successorList==[]):            
                ...
                if pathLengthDict[pathLength]<20:                   
                    allPathList.append(Path(currPath))
                ...
                if vtran==currTrans: ### find currTrans transition 's tranVarDict()
                    for ftran,fdict in self.currPathTranFuncDict.iteritems():
                        if ftran==currTrans:  ### find currTrans transition 's tranFuncDict()#                    
                            executActFlag=0
                            rename=0
                           ### '#### deal with condition variables######'
                            if (fdict['condFunc']!=[''] and fdict['condFunc']!=[]):
                                for var in vdict['condVuse']:                                    
                                    if var in actionVdefValueDict.keys():  ##var is defined in action                                  
                                        condVuseValueDict[var]=actionVdefValueDict[var]                                      
                                    if var in self.pathVarValue.keys(): ## var is a input variable
                                        condVuseValueDict[var]=self.pathVarValue[var]                                                                                 
                                    if self.originalDef.count(var)>1:                                  
                                    ##### condVuseValueDict[var] should be given the variable value from current event
                                        renameAndValue=self.findVarInEvent(var, fdict['eventFunc'])
                                        if renameAndValue!={}:
                                            rename=1
                                            condVuseValueDict[var]=renameAndValue[1]
                                    if tranExec[currTrans]>=1:                                   
                                        for varp,valuep in self.pathVarValue.iteritems():                                    
                                            check=tranExec[currTrans]
                                            check=currTrans+'_'+repr(check)
                                            if varp.count(check)>=1 and varp.find(var)>=0:
                                            #    print 'var varp, valuep',var, varp,valuep
                                                condVuseValueDict[var]=valuep
                                                break   
                                 ###### execute condition#########
                                isString=0
                                tempCondFun=[]
                                        ...
                                        for strj in actionStr.split("="):
                                            ...
                                            if strj!=stri:                                              
                                                ...
                                                for var in vdict['actionVuse']:
                                                    if var in actionVdefValueDict.keys():                                                
                                                        actionVuseValueDict[var]= actionVdefValueDict[var]                                                       
                                                    if var in self.pathVarValue.keys():
                                                        actionVuseValueDict[var]=self.pathVarValue[var]                                             
                                                    if self.originalDef.count(var)>1:
                                     ##### actionVuseValueDict[var] is given the variable value in current event
                                                        renameAndValue=self.findVarInEvent(var, fdict['eventFunc'])
                                                        if renameAndValue!={}:
                                                            actionVuseValueDict[var]=renameAndValue[1]
                                                    if tranExec[currTrans]>=1:                                                  
                                                        for varp,valuep in self.pathVarValue.iteritems():
                                                            check=tranExec[currTrans]
                                                            check=currTrans+'_'+repr(check)
                                                            if varp.count(check)>=1 and varp.find(var)>=0:                                                       
                                                                actionVuseValueDict[var]=valuep
                                                                break 
                                                actionVdefValueDict[stri]=eval(eval(rightStr),  {}, actionVuseValueDict)                                             
                                                oneActFlag=1
                                                break  ## break strj loop                                    
                                    if oneActFlag==1: break   #### break stri loop
                            executActFlag=1                         
            ...
            for repeat in range(10):
                ...
                for j in  range(maxGeneration):
                    ...
                    if invidualFitness[0]==0:                        
                        print '  Success generation2:',j,
                        testSuccess=1
                        numSuccess=0
                        break  # break for j loop
                    population=gaSample.basicSurvive(oldInvidualFit, invidualFitness,population)
              #      population=gaSample.elistSurvive(oldInvidualFit, invidualFitness,population)             
                        ### basic survive stratgy use old and new individual fitness
                if testSuccess==1:
                  ...
                  # break
                    ...
                    numfail=numfail+1
                    if (numfail>=2 ):
                        break   ##  5 fail, deal with next paht
            if numfail<2:
                self.pathTestGen[len(currPath)]=self.pathTestGen[len(currPath)]+1
        else:
             #   print 'no input variavle'
            noInputVar=1
...
################### find all path for a given transition###############
    ...
    def allPathNum(self):
        ...
        allpathlist=self.findAllPath()
#        for item in allpathlist:
#            print item
#        print '**************************'
        testpath=['T1','T2','T2']
        updatetranVarDict={}
        updatetranFuncDict={}
        index=0
        pathlen=len(testpath)
        while index<pathlen:
            updatetranVarDict[index]=self.tranVarDict[testpath[index]]
            updatetranFuncDict[index]=self.tranFuncDict[testpath[index]]
            index=index+1
        index=0
        while index<pathlen:
            print index,': ',updatetranVarDict[index],'\n\t',updatetranFuncDict[index]
            index=index+1
        print '*************************'
        index=0
        while index<pathlen:
            if updatetranVarDict[index]['eventVdef']!=[]:
                for var in updatetranVarDict[index]['eventVdef']:
                    i=index-1
                    while i>=0:
                        j=0
                        while j<len(updatetranVarDict[i]['eventVdef']):
                            if updatetranVarDict[i]['eventVdef'][j]==var:
                                updatetranVarDict[i]['eventVdef'][j]=var+'_'+str(i)
                                break
                            j=j+1
                        i=i-1
            index=index+1
...
#            self.vDefUseList(item)
#            print item.name,': ',self.tranVarDict[item.name],'\n\t',self.tranFuncDict[item.name]
#        for item in allpathlist:
#            print item
