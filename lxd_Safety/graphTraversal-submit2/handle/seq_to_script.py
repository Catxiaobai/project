    ...
    for info in range(len(infor)):
        ...
        if T_info[0] =="" and T_info[2] == "click":
            ...
            if temp[0] == "xpath" and len(t) == 1:  #最好click对象都是xpath的，看t的长度知道有几个参数，从而调用哪个脚本
                ...
                # else:
                #     case.webchess_T1(driver, t[0], t[1], "test", "123", temp[1])
...
def saveTargetBranch(driver, name):
    if name == tarGet.targetBranch().name:
    #if name == 'T16':
        print "获取目标分支页面"
        time.sleep(5)
        pageSource = driver.page_source
        with open(r'../efsmGA/states/' + name + '.html', 'w') as gf:
            # print pageSource
            gf.writelines(pageSource.encode('utf-8'))
...
def runcase(tevent, pathT, induval):
    ...
    ###################################
    # schoolmate程序需要确保是正确登录状态直接将个体所谓前两个参数设为test，1,使关于用户名密码的统统人为化，以便得以进入系统
    # case.dologin(driver, "3", "3")   #schoolmate程序的teacher身份登录
...
if __name__ == '__main__':
    ...
    # #webchess
    # pathT = ['T2','T3', 'T8', 'T16',]
    # induval = ['d807KZ', '4D67q4', 'wrI9b1', '96jg50', 'Z9ya5X', 'mAe01P', 'ATf9Sc', 'ktPD3G', 4462, '11U18c', '42qe42', 'w7fDoz', 2301]
