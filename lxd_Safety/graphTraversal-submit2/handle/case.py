...
#webchess的登录
def loginss(driver, user, psw): # 登录，参数是用户名和密码  T3
    driver.find_element_by_name("txtNick").clear()
    driver.find_element_by_name("txtNick").send_keys(user)
    # element is hidden ,must execute js to obtain
    # driver.execute_script("document.getElementById('input-login-password').style.display='block';")
    driver.find_element_by_name("pwdPassword").clear()
    driver.find_element_by_name("pwdPassword").send_keys(psw)
    driver.find_element_by_xpath("/html/body/div[2]/div/div[1]/form/div/div[6]/input[1]").click()
