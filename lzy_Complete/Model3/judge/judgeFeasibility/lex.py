...
__version__    = "3.7."
__tabversion__ = "3.7"       # Version of table file used
...
try:
    _INSTANCETYPE1=object()
    _INSTANCETYPE = (type(_INSTANCETYPE1),object)
    # isinstance(_INSTANCETYPE,object)
except AttributeError:
    # _INSTANCETYPE = types.InstanceType
    _INSTANCETYPE =type(_INSTANCETYPE1)
    class object: pass
...
class Lexer:
    ...
    def input(self,s):
        ...
        c = s[:1]
        if not (isinstance(c,bytes) or isinstance(c,str)):
...
# -----------------------------------------------------------------------------
def lex(module=None,object=None,debug=0,optimize=0,lextab="lextab",reflags=0,nowarn=1,outputdir=""):
    global lexer
    ldict = None
         ...
         if not (isinstance(states,tuple) or isinstance(states,list)):
              ...
              for s in states:
                    if not isinstance(s,tuple) or len(s) != 2:
                           ...
                           continue
                    name, statetype = s
                    if not isinstance(name,types.bytes):
                           print("lex: state name %s must be a string" % repr(name),file=sys.stderr)
                           error = 1
                           continue
                    if not (statetype == 'inclusive' or statetype == 'exclusive'):
                           print(sys.stderr, "lex: state type for state %s must be 'inclusive' or 'exclusive'" % name,file=sys.stderr)
                           error = 1
                           continue
                    if name in stateinfo:
                           print ("lex: state '%s' already defined." % name,sys.stderr)
                           error = 1
                           continue
    ...
    for f in funcsym.values():
        sorted(f, key=lambda x:x[1].__code__.co_firstlineno)

    # Sort the strings by regular expression length
    for s in strsym.values():
        sorted(s,key=lambda x:len(x[1]))
    ...
    for state in stateinfo.keys():
        ...
        for name,r in strsym[state]:
            ...
            if tokname == 'ignore':
                 if "\\" in r:
                      print(ys.stderr, "lex: Warning. %s contains a literal backslash '\\'" % name,file=sys.stderr)
                 ignore[state] = r
                 continue
            ...
            if not optimize:
                ...
                try:
                    ...
                    print(sys.stderr, "lex: Invalid regular expression for rule '%s'. %s" % (name,e),file=sys.stderr)
                    if '#' in r:
                         print(sys.stderr, "lex: Make sure '#' in rule '%s' is escaped with '\\#'." % name,file=sys.stderr)
        ...
        if not regex_list:
             print(sys.stderr, "lex: No rules defined for state '%s'" % state,file=sys.stderr)
             error = 1
