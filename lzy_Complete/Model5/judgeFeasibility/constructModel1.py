...
def getTransState1():
    ...
    lines = open('judgeFeasibility/Trace.txt', 'r', encoding="utf-8").readlines()
    states_label = []
    ...
    for line in lines:
        ...
        if fileds[0] == "source":
            ...
            source, event, condition, action, target = "", "", "", "", ""
    TraceSet.append(Trace)
    output = open('judgeFeasibility/TraceSet.txt', 'w+')
    for i in range(len(TraceSet)):
        ...
        output.write('\n')
    output.close()
    with open('judgeFeasibility/StateSet.txt', 'w') as f:
        json_str = json.dumps(StateSet, ensure_ascii=False, indent=0)
        f.write(json_str)
        f.write('\n')
    output = open('judgeFeasibility/TransSet.txt', 'w+')
...
def constructModel1():
    ...
    if flagValue:
        s = "S" + str(i)
        State3[flagValue]=s
    with open('judgeFeasibility/S1.txt', 'w') as f:
    ...
    for key, value in State3.items():
        State4[value] = key
    with open('judgeFeasibility/S2.txt', 'w') as f:
    ...
    for key,value in mergeCond.items():
        ...
        T1.append(mergeSame)
        k += 1
    output = open('judgeFeasibility/T6.txt', 'w+')
    ...
    # 生成PDF图
    # f = Digraph('digraph g', filename="judgeFeasibility/efsm1.gv")
    # s=""
    # for i in range(0, len(T1)):
    #     src = T1[i][1]
    #     if src== "S0":
