...
from judgeModelComplete import judgeModelComplete1
...
def completeModel1():
    ...
    if len(counterCondSet) > 0:
        if not os.path.isfile("E:/Code/Complete/Model3_2/Model3/addTrace.txt"):
            # 从键盘输入trace并保存到数组中
            # print("模型不完整，请补全：")
            ...
            # output.close()
            # # 从用户添加的文件中读取trace
            filename1 = "E:/Code/Complete/Model3_2/Model3/addTrace.txt"
            ...
            for line in lines:
                ...
                if fileds[0] == "source":
                    ...
                    newTrace2.append(line)
            with open('E:/Code/Complete/Model3_2/Model3/Trace1.txt', 'a', encoding='utf-8') as f:
                f.write('\n')
                for item in newTrace2:
                    f.write(item + " " + "\n")
            f.close()
            TraceSet, State, Trans, State2, T = ConstructModel1()
        elif os.path.isfile("E:/Code/Complete/Model3_2/Model3/addTrace.txt") and not os.path.getsize('E:/Code/Complete/Model3_2/Model3/addTrace.txt'):
            # print("请向文件添加入Trace：")
            # # 从键盘输入并存入addTrace.txt
            # newTrace = []
            # for line in iter(input, ''):
            ...
            f.close()
            TraceSet, State, Trans, State2, T = ConstructModel1()
        elif os.path.isfile("E:/Code/Complete/Model3_2/Model3/addTrace.txt") and os.path.getsize('E:/Code/Complete/Model3_2/Model3/addTrace.txt'):
            # 从用户添加的文件中读取trace
            sys.path.append("")
            writepath = r''
            filepath = r''
