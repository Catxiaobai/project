...
import math
import os
import efsmGA.GA.config as config
...
class GA:
    ...
    #    self.max=100  ### cruiseControl: max=100
        self.max=10000  ### Cashier, ATM: max=10000,
...
class SA:
    def __init__(self,solutionlength):
        self.solutionlength = solutionlength
        self.numsolutions = 20
        self.solutions = []
        self.min = 0
        self.max = 8191
        self.maxstr = 10
        self.minstr = 1
        self.step = int((self.max-self.min)*0.05)
    def createNewSolution(self, solutions, pathVarType):
        self.solutions = []
        if solutions == []:
            for i in range(self.numsolutions):
                self.solutions.append([])
                for j in range(self.solutionlength):
                    if pathVarType[j] == 'int':
                        self.solutions[i].append(random.randint(self.min, self.max))
                    elif pathVarType[j] == 'Boolean':
                        self.solutions[i].append(random.randint(0,1))
                    elif pathVarType[j] == 'string':
                        tempstr = ''
                        for k in range(random.randint(self.minstr, self.maxstr)):
                            tempstr = tempstr + chr(random.randint(97, 122))
                        self.solutions[i].append(tempstr)
        else:
            for i in range(self.numsolutions):
                self.solutions.append([])
                for j in range(self.solutionlength):
                    if pathVarType[j] == 'int':
                        self.solutions[i].append(solutions[i][j] ^ (2 ** random.randint(0, 12)))  # 12 represents max number 8191
                    elif pathVarType[j] == 'Boolean':
                        self.solutions[i].append(random.randint(0,1))
                    elif pathVarType[j] == 'string':
                        tempstr = ''
                        for k in range(random.randint(max(len(solutions[i][j]) - 1, self.minstr), min(len(solutions[i][j]) + 1, self.maxstr))):
                            tempstr = tempstr + chr(random.randint(97, 122))
                        self.solutions[i].append(tempstr)
...
class EFSM:
    ...
    def findStartTransition(self):
        ...
        startlist = ''
        for i in range(len(self.startTransitionList)):
            startlist = startlist + self.startTransitionList[i]
#        global information
#        information = information + '\tstart TransitionList ' + startlist + '\n'       
#        print '\tstartTransitionList ',self.startTransitionList
    ...
    def findEndTransition(self):
        ...
        endlist = ''
        for i in range(len(self.endTransitionList)):
            endlist =endlist + self.endTransitionList[i]
#        global information
#        information = information + '\tend TransitionList ' + endlist + '\n'
    ...
    def findAllPath(self):
        ...
        while len(tempPathList) > 0:
            ...
            if (successorList==[]):
                ...
                if len(allPathList)>1000:                  
                    break     
            tempPathList=restPathList + self.pathAppend(currPath, successorList)  
#        global innformation
#        innformation = information + '\t number of all path is ' + str(len(allPathList)) + '\n'
                    ...
                    for ftran,fdict in self.currPathTranFuncDict.iteritems():
                        if ftran==currTrans:  ### find currTrans transition 's tranFuncDict()#                    
                            ...
                            if self.tranVarDict[currTrans]['eventVdef'] != []:
                                for var in self.tranVarDict[currTrans]['eventVdef']:
                                    if currPath[0:i+1].count(currTrans) > 1:
                                        condVuseValueDict[var] = self.pathVarValue[var+'_'+currTrans+'_'+repr(currPath[0:i].count(currTrans))]
                                    elif self.originalDef.count(var) > 1:
                                        for item in vdict['eventVdef']:
                                            if var == item[0:len(var)] and len(item) == len(var)+1:    #may be will produce some errors, for example: accept0, accepts
                            ...
                            if fdict['condFunc']!=[''] and fdict['condFunc']!=[]:
                                ...
                                #isString=0
                                #tempCondFun=[]
                                #tempCondFun.extend(fdict['condFunc'])
                                #condStr=tempCondFun.pop()
                                #leftRightList=[]
                                #leftRightList=self.identifyLeftRight(condStr)
                                #left=leftRightList.pop(0)                           
                                #right=leftRightList.pop(0)                             
                                #left=left.strip("(")
                                #right=right.strip(")")                            
                                #if chr(34) in right or chr(39)in right:    # relate to char or string                                
                                #    if len(right)-2>1:
                                #       isString=1
                                #       if rename==1:
                                #           self.pathVarValue[tempsubsub]=right
                                #       else:
                                #           self.pathVarValue[left]=right                                   
                                #    else:                                                   
                                #       if (condVuseValueDict[left])<=255:                                  
                                #           condVuseValueDict[left]=chr(condVuseValueDict[left])                                           
                                #           condVuseTypeDict[left]="chr"                           
                                #if isString!=1:
...
#                return 0
    ...
    def obtainSolutionFitness(self,currPath,solution,noInputVar):
        self.pathVarValue={}
        for var, val in zip(self.pathDefVar, solution):
            self.pathVarValue[var]=val
        fitness=self.executePath(currPath, noInputVar)
        return fitness
...
#        invidualFitness={} ## key:genome, value:fitness
        noInputVar=0
        self.pathVarType = {}  ## key: variable , value: type of the variable 
        self.copyPathInfo()
#        lengthOfcurrPath=len(currPath)
...
#        print 'the currPath is '
#        print currPath
#        print 'self.originalDef'
#        print self.originalDef
#        print 'self.repeatTranVarDict'
#        print self.repeatTranVarDict
#        print 'self.repeatTranFuncDict'
#        print self.repeatTranFuncDict
#        print 'self.pathDefVar'
#        print self.pathDefVar
        
 #       print 'variables in this path:'
#        print 'self.currPathTranVarDict'
#        print self.currPathTranVarDict
#        print 'self.currPathTranFuncDict'
#        print self.currPathTranFuncDict
#        print 'self.tranVarDict'
#        print self.tranVarDict
 #       print '**********************************'
#        return 0
        ...
        for num in range(len(self.pathDefVar)):               
         ...
         #   if 'BrakeR' in self.pathDefVar[num] or 'AccelR' in self.pathDefVar[num]:
            if 'garauntee' in self.pathDefVar[num] or 'accept' in self.pathDefVar[num]:
           # if 'carwash' in self.pathDefVar[num] or 'Super' in self.pathDefVar[num]:
           #         or 'Medium' in self.pathDefVar[num] or 'Regular' in self.pathDefVar[num]:
                self.pathVarType[num] = 'Boolean'
            #elif 'pin' in self.pathDefVar[num] or 'P' in self.pathDefVar[num]:
            #    self.pathVarType[num] = 'string'
            else:
                self.pathVarType[num] = 'int'
#        information = 'currPath <**Path ['
#        for i in range(len(currPath)-1):
#            information = information + '\''+ currPath[i] + '\'' + ','
#        information = information + '\'' + currPath[len(currPath)-1] + '\']**>\t' + str(len(currPath)) + '\n'

#        print self.pathDefVar
        
#        print self.obtainSolutionFitness(currPath,[547, 6430, 15, 1, 0, 1, 1,1],noInputVar)
#        return 0
...
#            numandsumj=[0,0]
            for repeat in range(10):
                find = 0
                sa = SA(len(self.pathDefVar))
                temperature = len(currPath)
                solutions = sa.createNewSolution([],self.pathVarType)
                
                solutionsFitness = []
                for i in range(len(solutions)):
                    solutionsFitness.append(self.obtainSolutionFitness(currPath,solutions[i],noInputVar))

                minSolutionsFitness = len(currPath)
                for i in range(len(solutionsFitness)):
                    if minSolutionsFitness > solutionsFitness[i]:
                        minSolutionsFitness = solutionsFitness[i]
                bestFitness = minSolutionsFitness                    
                if minSolutionsFitness == 0:
                    find = 1
                    break
                ...
                iteration = 1
                while find == 0:
                    for i in range(32 * len(self.pathDefVar) * 10):
                        newSolutions = sa.createNewSolution(solutions, self.pathVarType)
                        newSolutionsFitness = []
                        for k in range(len(newSolutions)):
                            newSolutionsFitness.append(self.obtainSolutionFitness(currPath,newSolutions[k],noInputVar))

                        minNewSolutionsFitness = len(currPath)
                        for m in range(len(newSolutionsFitness)):
                            if minNewSolutionsFitness > newSolutionsFitness[m]:
                                minNewSolutionsFitness = newSolutionsFitness[m]
                            if newSolutionsFitness[m] == 0:
                                find = 1
                                solutionsFitness = newSolutionsFitness
                                break
                        if find == 1:
                            break
                        if minNewSolutionsFitness < minSolutionsFitness:
                            solutions = newSolutions
                            solutionsFitness = newSolutionsFitness
                            minSolutionsFitness = minNewSolutionsFitness
                        elif math.exp((minSolutionsFitness - minNewSolutionsFitness)/float(temperature)) > random.random():
#                        elif math.exp(-1/float(temperature)) > random.random():
                            solutions = newSolutions
                            solutionsFitness = newSolutionsFitness
                            minSolutionsFitness = minNewSolutionsFitness
                        iteration = iteration + 1
                    if bestFitness > minSolutionsFitness:
                        bestFitness = minSolutionsFitness
                        n = 0
                    else:
                        n = n + 1
                ...
                for j in range(len(solutionsFitness)):
                    if solutionsFitness[j] == 0:
                        solutionsFile.append(iteration)
                        break
            ...
            tempstr = tempstr + ']\t'
            tempstr = tempstr + str(len(currPath)) + '\n'
            
            if solutionsFile != []:
                j = 0
                for item in solutions:
                    tempstr = tempstr + 'No' + str(j) + '\tsuccessGeneration:\t' + str(item) + '\n'
                    j = j + 1
                path = os.getcwd()
                file1 = open(path + '/result.txt', 'a')
                file1.write(tempstr)
                file1.close()
            else:
                path = os.getcwd()
                file1 = open(path + '/result.txt', 'a')
                tempstr = tempstr + 'this path is not feasible\n'
                file1.write(tempstr)
                file1.close()
            '''
            return solutions[0]
...
#        return 0
        #i=0
        #currPath=pathList[i]
        currPath = ['T2', 'T3', 'T9', 'T22']
        self.testGenforPath(currPath)
        return 0
        print time.ctime()
        #while(len(currPath) <= 50):
        #    self.testGenforPath(currPath)
        #    i=i+1
        #    currPath=pathList[i]
        print time.ctime()
     #   CruiseControlinFeasiblePath=[['T1','T2','T3','T4','T8','T12','T3','T6','T10','T12','T4','T3','T6','T10','T14','T4','T6','T15'],\
     #                   ['T1','T2','T3','T4','T8','T12','T3','T6','T10','T12','T4','T3','T6','T10','T14','T5','T3','T8','T12','T3','T15']]
      #  CashierFeasiblePath=[['T1', 'T3', 'T12', 'T14'],\
             #            ['T1', 'T2', 'T3', 'T12', 'T14'],\
             #            ['T1', 'T2', 'T2', 'T3', 'T12', 'T14'],\
             #            ['T1', 'T2', 'T3', 'T16', 'T17', 'T19', 'T20'],\
             #            ['T1', 'T2', 'T2', 'T2', 'T2', 'T3', 'T12', 'T14'],\
             #            ['T1', 'T2', 'T2', 'T3', 'T16', 'T17', 'T18', 'T12', 'T14'],\
             #            ['T1', 'T2', 'T3', 'T12', 'T13', 'T16', 'T17', 'T18', 'T12', 'T14'],\
             #            ['T1', 'T2', 'T3', 'T4', 'T9', 'T10', 'T10', 'T10', 'T15', 'T12', 'T14'],\
             #            ['T1', 'T2', 'T2', 'T2', 'T3', 'T12', 'T13', 'T16', 'T17', 'T18', 'T12', 'T14'],\
             #            ['T1', 'T2', 'T3', 'T4', 'T9', 'T10', 'T10', 'T10', 'T15', 'T16', 'T17', 'T19', 'T20'],\
             #            ['T1', 'T2', 'T2', 'T3', 'T16', 'T17', 'T18', 'T16', 'T17', 'T18', 'T12', 'T13', 'T12', 'T14'],\
             #            ['T1', 'T2', 'T2', 'T3', 'T16', 'T17', 'T18', 'T16', 'T17', 'T18', 'T12', 'T13', 'T4', 'T9', 'T11'],\
             #            ['T1', 'T2', 'T2', 'T2', 'T3', 'T12', 'T13', 'T16', 'T17', 'T18', 'T12', 'T13', 'T4', 'T5', 'T6', 'T8'],\
             #            ['T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T16', 'T17', 'T18', 'T12', 'T13', 'T16', 'T17', 'T18', 'T12', 'T14'],\
             #            ['T1', 'T2', 'T2', 'T3', 'T16', 'T17', 'T18', 'T12', 'T13', 'T12', 'T13', 'T12', 'T13', 'T16', 'T17', 'T18', 'T4', 'T9', 'T11']]                      
        
#        l=['T1', 'T2', 'T2', 'T4', 'T5', 'T7', 'T9', 'T7', 'T12', 'T15', 'T13', 'T15', 'T9', 'T23']
#        l=['T1','T2','T4','T5','T7','T11','T16','T9','T23']
#        l1=['T1','T2','T3']
#        l2=['T1', 'T2', 'T2', 'T4', 'T5', 'T7', 'T9', 'T7', 'T12', 'T15', 'T13', 'T15', 'T9', 'T23']
#        currPath=Path(l2)
#        print self.pathTestGen
#        print '####################################'
#        for path in pathList:
#            self.pathTestGen[len(path)]=self.pathTestGen[len(path)]+1
#        print self.pathTestGen
#        print '#####################################'
#        templist=[0,0]
#        sumandnum={}
...
#        i=0
#        currPath=pathList[i]
#        numandsumj=self.testGenforPath(currPath)
#        while(len(currPath) <= 50):
#            print '*************************************************'
#            print 'test data generation path: '
#            print currPath,'\n',len(currPath)
#            numandsumj=self.testGenforPath(currPath)
#            self.testGenforPath(currPath)
#            if numandsumj[0] > 0:
#                dic[len(currPath)][0]=dic[len(currPath)][0]+1
#                dic[len(currPath)][1]=dic[len(currPath)][1]+numandsumj[2]
#            print '*************************************************'
#            i=i+1
#            currPath=pathList[i]
#        global information
...
#                information = information + str(i) + '\t' + str(dic[i][2]) + '\n'
#        info = information
#        information = ''
#        return info
#                print i,'\t',dic[i][2]
#        while (len(currPath)<=30):
#            length= len(currPath)
#            if  length >20 and self.pathTestGen[len(currPath)]<5:
#                self.DUAnalysisForPath(currPath)
#                self.testGenforPath(currPath)
#            i=i+1
#            currPath=pathList[i]
...
#            currPath=pathList[i]       
#            if currPath not in inFeasiblePath and self.pathTestGen[len(currPath)]==0:
#                self.DUAnalysisForPath(currPath)
#                self.testGenforPath(currPath)
#            i=i+1

#        currPath = ['T1', 'T2', 'T3', 'T4', 'T5','T6','T7','T16','T17','T19','T21','T16','T17','T19','T20']
#        self.testGenforPath(currPath)
...
#
        ...
        return SM
def obtain_efsm():
    inputfile = config.getExecuteModule()
    SM = efsmFromFile(inputfile)
    SM.addTransition(tarGet.targetBranch())
    SM.allPathNum()
