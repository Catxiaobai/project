...
from efsmGA.GA.search3 import PartialList
...
def fixmodel():
    ...
    for i in range(len(s) - 1):
        flag = 1  # 触发状态为新状态
        if isdomsame(i) == 1:  # 有相同状态
            flag = 0  # 不是新状态
            break
    starttime = time.time()
    target=PartialList(tarGet.targetBranch(),starttime).targetBranch
    target.tgt = s[-1]
