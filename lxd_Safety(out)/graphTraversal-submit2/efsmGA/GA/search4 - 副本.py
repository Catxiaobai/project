...
class PartialList:
    ...
    def getCandidatelist(self):
        ...
        if self.sortKaiGuan == 1 and self.tranlist[-1].sortedflag == 0:
            '''
            for item in tmplist:
            ...
            if len(atmplist) >= 1:  # 如果存在相关迁移，且原迁移未筛选过
                ...
                if config.achcdpx == 1:
                    tmplist = sorted(
                        tmplist,
                        key=lambda x:
                        (len(set(x.getveventVdef()) & set(self.useList))*self.defPercent[0]
                         + len(set(x.getvactionVdef()) & set(self.useList))*self.defPercent[1] + 1) / (
                                            len(self.useList) + 1), reverse=True
                    )  # 按重合程度排序
                if config.achxhpx == 1:
                    tmplist = sorted(tmplist, key=self.achxhpx)  # 按重合先后排序
                if config.hhpx == 1:
                    tmplist = sorted(tmplist, key=self.hhpx)  # 混合排序
    ...
    def hhpx(self, tran):  # 混合排序
        ...
        for item in reversed(self.tranlist):
            ...
            if len(defSet) == 0:  # 不存在 没在序列中找到定义的 def集合 时，break
                break
            res += config.distMap(dist) * config.numMap(len(defSet & set(item.getvUseList())))  # 距离的映射函数 * 数量的映射函数
        return dist
...
def search():
    ...
    while numOfPSG > 0:
        ...
        numOfPSG = numOfPSG - 1
        starttime = time.time()
        partialList = PartialList(config.getTargetBranch(), starttime)
        ...
        if partialList.targetBranchSrcList is not None:
            while partialList.isComplete() == 2 or (
                ...
                if partialList.sortKaiGuan == 1:
                    ...
                    if partialList.sortKaiGuan == 0 or \
                            (partialList.sortKaiGuan == 2 and candidateList[0].quality == 'noRelate'):
    ...
    for item in psDict.values():
        res += item
    print "序列生成平均值为%s" % (res / len(psDict))
    print "优先级排序时间: %s"%(partialList.sorttime)
    return [item.getName() for item in partialList.tranlist]
