...
FILENAME = 'E:/Code/project301/file/result.txt'
...
app.layout = html.Div([
    ...
    html.Div(id='message', style={'width': '50%', 'display': 'block', }, children=[
        html.P(children='', style={'color': 'green'}),
        html.Button(id='confirm', n_clicks=0, children='确认', style={'color': 'blue'}),
        html.Button(id='cancel', n_clicks=0, children='取消', style={'color': 'blue', 'marginLeft': '20px'}),
        html.Button(id='confirm_delete', n_clicks=0, children='确认', style={'color': 'blue'}),
        html.Button(id='cancel_delete', n_clicks=0, children='取消', style={'color': 'blue', 'marginLeft': '20px'}),
    ]),
    cyto.Cytoscape(
...
def update_charts(btn_node, btn_edge, btn_confirm, btn_cancel, btn_confirm_delete, btn_cancel_delete, node_data,
                  edge_data,
                  e_name, e_src, e_tgt,
                  e_event,
    ...
    if int(btn_confirm) > int(btn_cancel):
        data = read_txt('./check.txt')
    else:
        if first_time:
            data = read_txt("E:/Code/project301/file/result.txt")
        else:
            data = read_txt('./result1.txt')
    ...
    if int(btn_confirm) > int(btn_cancel):
        div_children = [
            ...
            html.Button(id='confirm_delete', n_clicks=0, children='确认',
                        style={'color': 'blue', 'visibility': "hidden"}),
            html.Button(id='cancel_delete', n_clicks=0, children='取消', style={'color': 'blue', 'visibility': "hidden"}),
        ]
        create_txt(data, checking=False)
        first_time = False
        return data, div_children
    elif int(btn_confirm) < int(btn_cancel):
        div_children = [
            ...
            html.Button(id='confirm_delete', n_clicks=0, children='确认',
                        style={'color': 'blue', 'visibility': "hidden"}),
            html.Button(id='cancel_delete', n_clicks=0, children='取消',
                        style={'color': 'blue', 'visibility': "hidden"}),
        ]
        return data, div_children
    else:
        if int(btn_cancel_delete) > int(btn_confirm_delete):
            div_children = [
                html.P(children='已取消删除', style={'color': 'green'}),
                html.Button(id='confirm', n_clicks=0, children='确认', style={'color': 'blue', 'visibility': "hidden"}),
                html.Button(id='cancel', n_clicks=0, children='取消', style={'color': 'blue', 'visibility': "hidden"}),
                html.Button(id='confirm_delete', n_clicks=0, children='确认',
                            style={'color': 'blue', 'visibility': "hidden"}),
            ...
            return data, div_children
        elif int(btn_confirm_delete) > int(btn_cancel_delete):
            check_data = read_txt('./check.txt')
            create_txt(check_data, checking=True)  # 创建check文件
            check_success = check()  # 检查是否可以执行
            if check_success == 'Yes':
                div_children = [
                    html.P(children='执行成功', style={'color': 'green'}),
                    html.Button(id='confirm', n_clicks=0, children='确认',
                                style={'color': 'blue', 'visibility': "hidden"}),
                    html.Button(id='cancel', n_clicks=0, children='取消',
                                style={'color': 'blue', 'visibility': "hidden"}),
                    html.Button(id='confirm_delete', n_clicks=0, children='确认',
                                style={'color': 'blue', 'visibility': "hidden"}),
                    html.Button(id='cancel_delete', n_clicks=0, children='取消',
                ...
                create_txt(check_data, checking=False)
                first_time = False
                return check_data, div_children
                ...
                div_children = [
                    ...
                    html.Button(id='confirm', n_clicks=0, children='确认',
                                style={'color': 'blue', 'visibility': "visible"}),
                    html.Button(id='cancel', n_clicks=0, children='取消',
                                style={'color': 'blue', 'visibility': "visible"}),
                    html.Button(id='confirm_delete', n_clicks=0, children='确认',
                                style={'color': 'blue', 'visibility': "hidden"}),
            ...
            if int(btn_edge) > int(btn_node) and e_name and e_src and e_tgt and e_event and e_condition and e_action:
                ...
                check_data = copy.deepcopy(data)
                check_data.append({"data": e_edge})
                create_txt(check_data, checking=True)  # 创建check文件
                check_success = check()  # 检查是否可以执行
                if check_success == 'Yes':
                    div_children = [
                ...
                check_data = copy.deepcopy(data)
                check_data.append({"data": {"id": n_label, "label": n_label, 'name': n_name, "color": "gray"}})
                create_txt(check_data, checking=True)  # 创建check文件:
                check_success = check()  # 检查是否可以执行
                if check_success == 'Yes':
                    div_children = [
                ...
                if node_data:
                    ...
                    check_data.remove(dict_node_data)
                    create_txt(check_data, checking=True)
                    div_children = [
                        html.P(children='确认删除此节点吗', style={'color': 'green'}),
                        html.Button(id='confirm', n_clicks=0, children='确认',
                                    style={'color': 'blue', 'visibility': "hidden"}),
                        html.Button(id='cancel', n_clicks=0, children='取消',
                                    style={'color': 'blue', 'visibility': "hidden"}),
                    ...
                    check_data.remove(edge_node_data)
                    create_txt(check_data, checking=True)
                    div_children = [
                        html.P(children='确认删除这条边吗', style={'color': 'green'}),
                        html.Button(id='confirm', n_clicks=0, children='确认',
                                    style={'color': 'blue', 'visibility': "hidden"}),
                        html.Button(id='cancel', n_clicks=0, children='取消',
                                    style={'color': 'blue', 'visibility': "hidden"}),
